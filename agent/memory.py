"""
GraphRAG and Case Memory Engine
Manages historical case memory (5,565 closed cases), policy retrieval,
and regulatory guidance for grounding agent decisions.
"""
...
class CaseMemoryRAG:
    def __init__(self, data_dir: str = "data"):
        ...
        self.policy_rules = {
            "R1": "Verify before you block on a weak signal. If the case rests on a single signal (including a risk score alone) and assessed fraud probability is below 0.70, recommend VERIFY_WITH_CUSTOMER or STEP_UP_AUTH before any block. Blocking a legitimate customer on one signal is a policy breach.",
    ...
    def retrieve_similar_cases(self,
        ...
        """
        Retrieves top similar historical cases from memory.
        Prioritizes:
        1. Prior closed cases for this exact customer
        2. Closed cases with identical confirmed pattern and similar exposure
        """
        results = []
        seen_case_ids = set()

        # 1. Customer's historical cases
        cust_cases = self.closed_cases_df[self.closed_cases_df['customer_id'] == customer_id]
        for _, row in cust_cases.iterrows():
            cid = row['case_id']
        ...
        # 2. Pattern-based historical cases
        if len(results) < limit and pattern and pattern != "none":
            pattern_cases = self.closed_cases_df[self.closed_cases_df['pattern'] == pattern]
            for _, row in pattern_cases.sort_values('exposure_usd', ascending=False).iterrows():
                cid = row['case_id']
                if cid not in seen_case_ids:
                    seen_case_ids.add(cid)
                    results.append(row.to_dict())
    ...
    def retrieve_policy_context(self, active_rules: List[str]) -> str:
        """Constructs policy context text for the LLM / reasoning engine."""
        lines = ["--- APPLICABLE FRAUD POLICY RULES ---"]
        for r in active_rules:
            if r in self.policy_rules:
                lines.append(f"[{r}]: {self.policy_rules[r]}")
        return "\n".join(lines)
