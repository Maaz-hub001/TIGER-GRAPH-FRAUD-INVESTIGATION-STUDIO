...
async function loadSystemStatus() {
    try {
        ...
        const statusText = document.getElementById("engineStatusText");
        statusText.textContent = "TigerGraph GSQL Engine: Online (26,643 Txns / 144,432 Devs)";
    }
}
...
async function loadCasesList() {
    try {
        ...
        try {
            ...
            res = await fetch("data/cases_summary.json");
        }
        if (!res || !res.ok) {
            res = await fetch("data/cases_summary.json");
        }
...
async function selectCase(caseId) {
    ...
    try {
        ...
        try {
            ...
            const graphRes = await fetch(`/api/graph/${caseId}`);
            if (graphRes.ok) graphData = await graphRes.json();
        } catch (_) {}

        // Fallback to static case JSON if running on static host
        if (!caseData) {
            const caseRes = await fetch(`cases/${caseId}.json`);
            caseData = await caseRes.json();
        }

        if (!graphData && caseData) {
            graphData = buildGraphDataFromCase(caseId, caseData);
        }
...
function buildGraphDataFromCase(caseId, caseData) {
    const caseObj = caseData.case;
    const custId = caseData.customer_id || (caseData.case.summary ? (caseData.case.summary.match(/Customer (C\d+)/) || [])[1] : null) || ("C" + caseId.replace("HHG-", "00")) || "C12382";
    const cardId = caseData.card_id || (caseData.case.summary ? (caseData.case.summary.match(/card (C\d+-K\d+)/) || [])[1] : null) || `${custId}-K1`;
    const flgTxn = caseObj.first_suspicious_txn_id || (caseObj.affected_txn_ids && caseObj.affected_txn_ids[0]) || "3500000";
    ...
    edges.push({ from: custId, to: cardId, label: "OWNS", color: { color: "#64748b" } });

    // 3. Flagged Transaction Node
    const isFraud = caseObj.verdict === "fraud";
    const flgColor = isFraud ? "#ef4444" : "#10b981";
    nodes.push({
        id: flgTxn,
        label: `Flagged Txn\n#${flgTxn}\n$${(caseObj.exposure_usd || 0).toFixed(2)}`,
        group: "transaction",
        shape: "dot",
        size: 26,
        color: flgColor,
    ...
    edges.push({ from: cardId, to: flgTxn, label: "MADE", color: { color: "#f59e0b" } });

    // 4. Device Profile Node
    const devProfiles = caseObj.connected_device_profiles || [];
    if (devProfiles.length > 0) {
        const devId = `DEV-${caseId}`;
        const devText = devProfiles[0];
        const shortDev = devText.includes(" | ") ? devText.split(" | ")[0] : devText;
        nodes.push({
            id: devId,
        ...
        edges.push({ from: flgTxn, to: devId, label: "FROM_DEVICE", color: { color: "#06b6d4" } });

        // 5. Connected Cards (Syndicate Ring Nodes if present)
        const connCards = caseObj.connected_card_ids || [];
        if (connCards.length > 0) {
            const ringId = `RING-${caseId}`;
            nodes.push({
                id: ringId,
                label: `Syndicate Ring\n(${connCards.length} Cards)`,
                group: "syndicate",
                shape: "diamond",
                size: 32,
                color: "#f43f5e",
            ...
            edges.push({ from: devId, to: ringId, label: "EXPLOITS", color: { color: "#f43f5e" }, dashes: true });

            connCards.slice(0, 8).forEach(cc => {
                nodes.push({
                    id: cc,
                    label: cc,
                ...
                edges.push({ from: ringId, to: cc, color: { color: "#f43f5e" } });
            });
        }
    }

    // 6. FraudCase Graph Writeback Node
    if (caseObj.graph_case_id) {
        nodes.push({
            id: caseObj.graph_case_id,
            label: `Graph Case\n${caseObj.graph_case_id}`,
            group: "fraud_case",
            shape: "square",
            size: 20,
            color: "#eab308",
        ...
        edges.push({ from: caseObj.graph_case_id, to: cardId, label: "INVESTIGATES", color: { color: "#eab308" } });
        edges.push({ from: caseObj.graph_case_id, to: flgTxn, label: "FLAGS", color: { color: "#eab308" } });
    }
